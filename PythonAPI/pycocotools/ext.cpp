...
struct anns_struct {
	...
	// segmentation
	vector<vector<double>> segm_list;
	vector<int> segm_size;
	vector<int> segm_counts_list;
	string segm_counts_str;
};
...
struct detection_struct {
	...
	vector<vector<int>> segm_size;
	vector<int> ignore;
	vector<string> segm_counts;
};

// create index results
vector<int64_t> imgids;
vector<int64_t> catids;
vector<image_struct> imgsgt;
vector<vector<double>> gtbboxes;
vector<vector<RLE>> gtsegm;

vector<detection_struct> gts;
vector<detection_struct> dts;

// internal computeiou results
vector<vector<double>> ious_map;
...
vector<size_t> sort_indices(const vector<T>& v, Comparator comparator = Comparator()) {
...
template <typename T, typename Comparator = greater<T> >
vector<size_t> stable_sort_indices(const vector<T>& v, Comparator comparator = Comparator()) {
		vector<size_t> indices(v.size());
		iota(indices.begin(), indices.end(), 0);
...
void accumulate(int T, int A, const int* maxDets, const double* recThrs,
				...
				const vector<vector<int64_t>>& gtignore,
				const vector<vector<double>>& dtignore,
				const vector<vector<double>>& dtmatches,
				const vector<vector<double>>& dtscores);

void compute_iou(string iouType, int maxDet, int useCats, int nthreads);

static PyObject* cpp_evaluate(PyObject* self, PyObject* args) {

	// there must be at least 1 process to run this program
	if (num_procs<1){
		cout << "[cpp_evaluate] Error: num_procs must be >=1" << endl;
		return NULL;
	}
	...
	if (!PyArg_ParseTuple(args, "iO!O!O!O!si|", &useCats, 
		...
		cout << "[cpp_evaluate] Error: can't parse arguments (must be int, numpy.ndarray, numpy.ndarray, numpy.ndarray, numpy.ndarray,str, int)" << endl;
		return NULL;
	}
	string iouType(iouType_chars);
	if (useCats<=0){
		cout << "[cpp_evaluate] Error: useCats must be >0" << endl;
		return NULL;
	}
	...
	for(npy_intp a = 0; a < pareaRngs->dimensions[0]; a++) {
		for(size_t c = 0; c < catids.size(); c++) {
			...
			vector<vector<int64_t>> gtIgnore_list;
			vector<vector<double>> dtIgnore_list;
			vector<vector<double>> dtMatches_list;
			vector<vector<double>> dtScores_list;
			...
			for(size_t i = 0; i < imgids.size(); i++) {
				...
				const int D = min(Do, maxDet);
				const int I = (G==0||D==0) ? 0 : D;
				
				// arrays
				vector<double> gtm(T*G, 0.0);
				gtIgnore_list.push_back(vector<int64_t>(G));
				dtIgnore_list.push_back(vector<double>(T*D, 0.0));
				dtMatches_list.push_back(vector<double>(T*D, 0.0));
				dtScores_list.push_back(vector<double>(D));
				...
				// get sorting indices
				auto gtind = sort_indices(gtIg, less<double>());
...
template <typename T>
vector<T> assemble_array(const vector<vector<T>>& list, size_t nrows, size_t maxDet, const vector<size_t>& indices) {
  vector<T> q;
  ...
  vector<T> res(indices.size() * nrows);
  for (size_t i = 0; i < nrows; ++i) {
                ...
                const vector<vector<double>>& dtscores) 
{
	...
	for (int m = 0; m < M; ++m) {
		...
		for (size_t e = 0; e < dtscores.size(); ++e) {
			...
			for (size_t j = 0; j < min(score.size(), static_cast<size_t>(maxDets[m])); ++j) {
		...
		vector<double> dtScoresSorted(dtScores.size());
		for (size_t j = 0; j < indices.size(); ++j) {
		...
		vector<double> tp_sum(nd * nrows);
		vector<double> fp_sum(nd * nrows);
		vector<double> rc(nd);
		vector<double> pr(nd);
...
void compute_iou(string iouType, int maxDet, int useCats, int nthreads) {
	...
	if (ious_map.size()>0){
		if (proc_id == 0) cout << "[compute_iou] IoUs already exist. Clearing vectors..." << endl;
		ious_map.clear();
	}
	...
	for(size_t c = 0; c < catids.size(); c++) {
		for(size_t i = 0; i < imgids.size(); i++) {
			...
			const int m = min(D, static_cast<size_t>(maxDet));
			const int n = G;
			
			if(m==0 || n==0) {
				continue;
			}
			ious_map[c*imgids.size() + i] = vector<double>(m*n);
...
string rleToString( const RLE *R ) {
...
string frUncompressedRLE(vector<int> cnts, vector<int> size, int h, int w) {
...
string frPoly(vector<vector<double>> poly, int h, int w) {
  size_t n = poly.size();
  RLE *Rs;
  rlesInit(&Rs,n);
...
unsigned int area(vector<int>& size, string& counts) {
  // _frString
  RLE *Rs;
  rlesInit(&Rs,1);
...
vector<float> toBbox(vector<int>& size, string& counts) {
  // _frString
  RLE *Rs;
  rlesInit(&Rs,1);
  //char *str = new char[counts.length() + 1];
...
void annToRLE(anns_struct& ann, vector<vector<int>> &size, vector<string> &counts, int h, int w) {
  auto is_segm_list = ann.segm_list.size()>0;
  auto is_cnts_list = is_segm_list ? 0 : ann.segm_counts_list.size()>0;

  if (is_segm_list) {
    vector<int> segm_size{h,w};
...
static PyObject* cpp_create_index(PyObject* self, PyObject* args) 
{
	 ...
	 * 		'segmentation': {'size': [int, int], 'counts': string} 
	 * */ 

	const char *annotation_file;
	int nthreads;
	if (!PyArg_ParseTuple(args, "siii|", &annotation_file, &num_procs, &proc_id, &nthreads)) {
		cout << "[cpp_create_index] Error: can't parse the argument (must be string)" << endl;
		return NULL;
	}
	// there must be at least 1 process to run this program
	if (num_procs<1){
		cout << "[cpp_create_index] Error: num_procs must be >=1" << endl;
		return NULL;
	}
	
	if (imgsgt.size()>0 || imgids.size()>0 || catids.size()>0) {
		if (proc_id == 0) cout << "[cpp_create_index] Ground truth annotations already exist. Clearing vectors..." << endl;
		imgsgt.clear();
		imgids.clear();
		catids.clear();
		gtbboxes.clear();
	...
	dom::parser parser;
	dom::element dataset = parser.load(annotation_file);

	dom::array imgs = dataset[kImages];
	for (dom::object image: imgs) {
		image_struct img;
		img.id = image[kId]; 
	...
	dom::array cats = dataset[kCats];
	vector<int64_t> catids_tmp;
	for (dom::object cat: cats) {
	...
	catids = vector<int64_t>(catids_tmp.begin()+begin, catids_tmp.begin()+end);
	...
	//vector<int64_t> cat_num_anns(catids_tmp_size, 0);
	//vector<int64_t> sum_anns_per_proc(num_procs, 0);
	//for (dom::object annotation : anns) {
	...
	//	cat_num_anns[catid]++;
	//}
	//auto cat_num_anns_inds = sort_indices(cat_num_anns, greater<int64_t>());
	//int argmin;
	//for (int64_t i=0; i<catids_tmp_size; i++){
	//	argmin = distance(sum_anns_per_proc.begin(), min_element(sum_anns_per_proc.begin(),sum_anns_per_proc.end()));
	//	if (argmin == proc_id) {
...
static PyObject* cpp_load_res_numpy(PyObject* self, PyObject* args) 
{
	...
	// there must be at least 1 process to run this program
	if (num_procs<1){
		cout << "[cpp_load_res_numpy] Error: num_procs must be >=1" << endl;
		return NULL;
	}
	...
	if (!PyArg_ParseTuple(args, "O!i|", &PyArray_Type, &anns, &nthreads)){
		cout << "[cpp_load_res_numpy] Error: can't parse the argument (must be numpy.ndarray)" << endl;
		return NULL;
	}
	//if (!PyArray_ISCARRAY_RO(anns)){
	//    cout << "[cpp_load_res_numpy] Input array is not CARRAY_RO. Making sure it's C_CONTIGUOUS and ALIGNED..." << endl;
	//    PyObject* anns_tmp = PyArray_FROM_OF(anns, NPY_ARRAY_C_CONTIGUOUS | NPY_ARRAY_ALIGNED);
	//    anns = anns_tmp;
	//    Py_XDECREF(anns_tmp);
	//}
	int ndim = anns->nd;
	...
	if (ndim != 2 || dim2 != 7){
		cout << "[cpp_load_res_numpy] Error: Input array must be 2-d numpy array with 7 columns" << endl;
		return NULL;
	}

	if (dts.size()>0) {
		if (proc_id == 0) cout << "[cpp_load_res_numpy] Detection annotations already exist. Clearing vectors..." << endl;
		dts.clear();
	}
...
static PyObject* cpp_load_res_json(PyObject* self, PyObject* args) 
{
	...
	// there must be at least 1 process to run this program
	if (num_procs<1){
		cout << "[cpp_load_res_json] Error: num_procs must be >=1" << endl;
		return NULL;
	}
	...
	if (!PyArg_ParseTuple(args, "si|", &annotation_file, &nthreads)){
		cout << "[cpp_load_res_json] Error: can't parse the argument (must be a .json file)" << endl;
		return NULL;
	}

	if (dts.size()>0) {
		if (proc_id == 0) cout << "[cpp_load_res_json] Detection annotations already exist. Clearing vectors..." << endl;
		dts.clear();
	}
	dts.resize(catids.size()*imgids.size());
	...
	if (isbbox){
		for (dom::object annotation : anns) {
			...
			dom::array ann_bbox= annotation[kBbox];
	    	vector<float> bbox;
			for (int d=0; d<4; d++)
		...
		for (dom::object annotation : anns) {
			...
			anns_struct ann;
			dom::object ann_segm = annotation[kSegm];
			for (int64_t size : ann_segm[kSize])
				ann.segm_size.push_back(static_cast<int>(size));
			ann.segm_counts_str = ann_segm[kCounts];
